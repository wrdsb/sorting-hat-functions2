...
const membershipsLegacyJCCalculate: AzureFunction = async function (context: Context, triggerMessage: string): Promise<void> {
    const functionInvocationID = context.executionContext.invocationId;
    const functionInvocationTime = new Date();
    const functionInvocationTimestamp = functionInvocationTime.toJSON();  // format: 2012-04-23T18:25:43.511Z

    const functionName = context.executionContext.functionName;
    const functionEventType = 'WRDSB.SortingHat.Memberships.LegacyJC.Calculate';
    const functionEventID = `sorting-hat-functions-${functionName}-${functionInvocationID}`;
    const functionLogID = `${functionInvocationTime.getTime()}-${functionInvocationID}`;

    const logStorageAccount = process.env['storageAccount'];
    const logStorageKey = process.env['storageKey'];
    const logStorageContainer = 'function-memberships-legacy-jc-calculate-logs';
    ...
    const legacyJCSets = context.bindings.legacyJCSets;
    ...
    async function calculateMembers (rows) {
        ...
        legacyJCSets.forEach(function(set) {
            members[set.id] = [];
        });
        ...
        rows.forEach(function(row) {
            if (row.EMAIL_ADDRESS
                ...
                legacyJCSets.forEach(function(set) {
                    if (set.definition[0].includes(job_code)) {
...
export default membershipsLegacyJCCalculate;
